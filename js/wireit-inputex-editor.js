/* New functions by ceichinger //ce */

function changeNoderuleType(container, fieldValue){
	console.log(container);
	switch (fieldValue){
		case 'Headquarter' : setSelectorHeadquarter(container); break;
		case 'AssetType' : setSelectorAssetType(container); break;
		case 'Headquarter_AssetType' : setSelectorHQ_AssetType(container); break;
		case 'Asset' : setSelectorAsset(container); break;
		case 'Kein' : setSelectorNone(container); break;
	}
	//$('.WireIt-Container-focused > .body > div > fieldset > div > div > select[name="SelectorType"]').val(fieldValue);
}

function setSelectorHeadquarter(container){
    if(container.selector.children.length > 0){
	for(var i = 1; i < container.selector.children.length; i++){
		container.selector.children[i].hidden = true;
	}
	container.selector.children[0].hidden = false;
    }
}

function setSelectorAssetType(container){
    if(container.selector.children.length > 0){
	for(var i = 0; i < container.selector.children.length; i++){
		container.selector.children[i].hidden = true;
	}
	container.selector.children[1].hidden = false;
    }
}

function setSelectorHQ_AssetType(container){
    if(container.selector.children.length > 0){
	for(var i = 2; i < container.selector.children.length; i++){
		container.selector.children[i].hidden = true;
	}
	container.selector.children[0].hidden = false;
	container.selector.children[1].hidden = false;
...
function setSelectorAsset(container){
    if(container.selector.children.length > 0){
	for(var i = 0; i < container.selector.children.length; i++){
		container.selector.children[i].hidden = true;
	}
	container.selector.children[2].hidden = false;
    }
}

function setSelectorNone(container){
    if(container.selector.children.length > 0){
	for(var i = 0; i < container.selector.children.length; i++){
		container.selector.children[i].hidden = true;
	}
...
/*
function changeContainerColor(container, fieldValue){
	var color = 'blue';
	switch (fieldValue){
		case 'Headquarter' : color = 'blue'; break;
		case 'AssetType' : color = 'yellow'; break;
		case 'Headquarter_AssetType' : color = 'green'; break;
		case 'Asset' : color = 'red'; break;
	}
	$('.WireIt-Container-focused > .WireIt-Container-ddhandle').css("background-color", color);
	$('.WireIt-Container-focused > .body').css("border-color",color);
}
*/

function showConnectableTerminals(srcTerminal, show){
	show = show == undefined? true : show;
	for(container in layer.containers){
		for(terminal in layer.containers[container].terminals){
		  if( srcTerminal.isValidWireTerminal(layer.containers[container].terminals[terminal].dd) ) {
	        layer.containers[container].terminals[terminal].setDropInvitation(show);
	      }
		}
	}
}
	...
	getConfig: function() {
      var obj = {
		...
		if(this.labelEditor) {
			...
			obj.labelEditor = labelEditor;//ce 
		}
...
lang.extend(WireIt.TerminalProxy, YAHOO.util.DDProxy, {
	 ...
	 */
	startDrag: function() {
		showConnectableTerminals(this); //ce: ermittle alle passenden Terminals
	...
	onDrag: function(e) {
	   ...
	   if(this.terminal.container) {
		var obj = this.terminal.container.layer.el;
         	...
         	var curtop = 0;
         	// ce issue #54 if (obj.offsetParent) => (!obj.offsetParent){
		// rollback of #54 solution because fakeTerminal is wrong placed if horizontal slider doesn't
	 ...
	 */
	endDrag: function(e) {
		showConnectableTerminals(this, false); //ce: ermittle alle passenden Terminals & beende das Highlighten
	   if(this.editingWire) {
	...
	onDragOut: function(e,ddTargets) { 
	   ...
	   for(var i = 0 ; i < ddTargets.length ; i++) {
	      if( this.isValidWireTerminal(ddTargets[i]) ) {
	         //ce ddTargets[i].terminal.setDropInvitation(false);
	      }
	   }
	},
	...
	isValidWireTerminal: function(DDterminal) {
	   ...
	   if( !DDterminal.isWireItTerminal ) {
		...
		// Wenn kein Problem auftritt, teste anders herum.
		var matched = matchTypeAllowedTypes(this, DDterminal);
		if(matched) matched = matchTypeAllowedTypes(DDterminal, this);
		if(!matched) {
			return false;
		}
		
		function matchTypeAllowedTypes(terminal1, terminal2){
			if(terminal1.termConfig.type && terminal2.termConfig.allowedTypes){
				if(lang.isString(terminal2.termConfig.allowedTypes)){
					var allowedRegExp = new RegExp(terminal2.termConfig.allowedTypes,'i');
					if(!allowedRegExp.test(terminal1.termConfig.type)) 
						return false;
				}  
				else if( WireIt.indexOf(terminal1.termConfig.type, terminal2.termConfig.allowedTypes) == -1 ) {
					return false;
				}
			} else if (terminal1.termConfig.type && !terminal2.termConfig.allowedTypes){
				return false;
			}
			return true;
		}
		...
		// Konvention es dürfen nicht beide AllowedTypes haben
		if(this.termConfig.allowedTypes && DDterminal.termConfig.allowedTypes){
			return false;
		}
...
WireIt.Terminal.prototype = {
   ...
   setOptions: function(options) {
      for(var k in options) {
		...
		if(options.isXnode){
			this.className = "WireIt-Terminal-X";
			this.connectedClassName = "WireIt-Terminal-connected-X";
			this.dropinviteClassName = "WireIt-Terminal-dropinvite-X";
		}
   },
...
WireIt.Container.prototype = {
    ...
    */
	...
	/** 
    * @property help
    * @description display a button to help the container
    * @default true
    * @type String
    */
	help: "",
    ...
    */
	...
	/** 
    * @property helpButtonClassName
    * @description CSS class name for the help button
    * @default "WireIt-Container-helpbutton"
    * @type String
    */
	helpButtonClassName: CSS_PREFIX+"Container-helpbutton",
    ...
    */
	...
	/** 
    * @property denotationClassName
    * @description CSS class name for the help button
    * @default "WireIt-Container-helpbutton"
    * @type String
    */
	denotationClassName: CSS_PREFIX+"Container-denotation",
   ...
   render: function() {
	   /** //ce
		* If the container has no ID, it sets an auto ID for the Container
		* Note: The ID is negativ, so own IDs can be given without any conflicts
		*/
	  if(this.id == null){
		this.id = -this.layer.nextContainerID;
		this.layer.nextContainerID += 1;
	  }
      ...
      if(this.ddHandle) {
		 ...
		 //ce Denotation
		 this.denotation = WireIt.cn('input', {className: this.denotationClassName} );
			if (this.ddHandle) {
				this.ddHandle.appendChild(this.denotation);
			}
			else {
				this.el.appendChild(this.denotation);
			}
         Event.addListener(this.denotation, "click", this.focusDenotation, this, true); 
      }
		
	  Event.addListener(this.el, 'mousedown', this.onMouseDown, this, true);
	  Event.addListener(this.el, 'mouseup', this.onMouseUp, this, true);
	  Event.addListener(this.el, 'mousemove', this.onMouseMove, this, true);
	  Event.addListener(this.el, 'click', this.onMouseClick, this, true);   
      ...
      if(this.help != "" || this.helpurl != "") {
         // Help button
         this.helpButton = WireIt.cn('div', {className: this.helpButtonClassName} );
			if (this.ddHandle) {
				this.ddHandle.appendChild(this.helpButton);
			}
			else {
				this.el.appendChild(this.helpButton);
			}
         Event.addListener(this.helpButton, "click", this.onHelpButton, this, true);
      }
   ...
   onMouseMove: function(e, args) { 
	 ...
	 for(var i = 0; i < this.layer.focusedContainer.length; i++){
		if(this.layer.focusedContainer[i] == this) {
			continue;
		}
 
		this.layer.focusedContainer[i].position[0] = this.layer.focusedContainer[i].startX + offsetX;
		this.layer.focusedContainer[i].position[1] = this.layer.focusedContainer[i].startY + offsetY; 
	
		var posXpx = this.layer.focusedContainer[i].startXpx + offsetX;
		var posYpx = this.layer.focusedContainer[i].startYpx + offsetY;
		
		posXpx = posXpx < 0 ? 0 : posXpx;
		posYpx = posYpx < 0 ? 0 : posYpx;
		
		//ce jquery
		$('#' + this.layer.focusedContainer[i].dd.id).css({"top":posYpx+"px", "left": posXpx+"px"});
		this.layer.focusedContainer[i].redrawContainerWires();
	 }

	 //ce scrollLayer
	 //this.layer.el.scrollLeft =  2000;
	 //this.layer.el.scrollTop =  1000;
	 
	 //if(this.el.offsetHeight + this.el.offsetTop > this.el.offsetParent.clientHeight - 20) {
	//	this.layer.el.scrollTop = this.el.offsetTop - (this.el.offsetParent.clientHeight / 2);
		//this.layer.el.scrollLeft = this.el.offsetLeft - (this.el.offsetParent.clientWidth / 2);
		//e.clientX =100;
		//console.log(this.layer.el.scrollTop);
	 //}
	 
   },   
   
   
   redrawContainerWires: function(){
	var terminalList = YAHOO.lang.isArray(this.dd._WireItTerminals) ? this.dd._WireItTerminals : 			(this.dd._WireItTerminals.isWireItTerminal ? [this.dd._WireItTerminals] : []);
		  // Redraw all the wires
		  for(var i = 0 ; i < terminalList.length ; i++) {
				if(terminalList[i].wires) {
					for(var k = 0 ; k < terminalList[i].wires.length ; k++) {
   ...
   onMouseDown: function(event) {
      this.setFocusedNewest();
	  var index = WireIt.indexOf(this, this.layer.focusedContainer);
	  if( index == -1 ) {
		if(!event.ctrlKey){
			for(var i = 0; i < this.layer.focusedContainer.length; i++){
				this.layer.focusedContainer[i].removeFocus();
				this.layer.focusedContainer[i] = null;
			}
			this.layer.focusedContainer = WireIt.compact(this.layer.focusedContainer);
		}
		this.setFocus();
		this.layer.focusedContainer.push(this);
		this.focusedOnMouseDown = true;
	  } else this.focusedOnMouseDown = false;
	  
	  for(var i = 0; i < this.layer.focusedContainer.length; i++){
		if(this.layer.focusedContainer[i] == this) continue;
		this.layer.focusedContainer[i].startX = this.layer.focusedContainer[i].position[0];
		this.layer.focusedContainer[i].startY = this.layer.focusedContainer[i].position[1];
		var position = $('#' + this.layer.focusedContainer[i].dd.id).position();
		this.layer.focusedContainer[i].startXpx = position.left;
		this.layer.focusedContainer[i].startYpx = position.top;
	  }
   ...
   onMouseClick: function(event) {
      if(new Date().getTime() - this.mousetime > 300) return;
      if(this.layer) {
		  if(event.ctrlKey){
			 var index = WireIt.indexOf(this, this.layer.focusedContainer);
			  // already focused -> remove Focus
			  if( !this.focusedOnMouseDown) {
				this.removeFocus();
				this.layer.focusedContainer[index] = null;
				this.layer.focusedContainer = WireIt.compact(this.layer.focusedContainer);	
			 } else if(index ==-1){ // set Focus
				this.setFocus();
				this.layer.focusedContainer.push(this);
			 } 
		 }else{
			// removeFocus of all focused containers
			for(var i = 0; i < this.layer.focusedContainer.length; i++){
				this.layer.focusedContainer[i].removeFocus();
				this.layer.focusedContainer[i] = null;
			}
			this.layer.focusedContainer = WireIt.compact(this.layer.focusedContainer);
			// Focus the selected container
			this.setFocus();
			this.layer.focusedContainer.push(this);
		}
	  }
   ...
   /** //ce
    * Called from onMouseDown in a Container and takes the current in top of all other containers
    */
   setFocusedNewest: function(){
		//ce jquery
		$('div.WireIt-Container-focused-newest').removeClass("WireIt-Container-focused-newest");
		Dom.addClass(this.el, CSS_PREFIX+"Container-focused-newest");
   },
   
   /** ce 
    * Called when the user clicked in the denotation input
    * @method focusDenotation
    */
   focusDenotation: function(e, args) {
	  if(new Date().getTime() - this.mousetime > 300) return;
      Event.stopEvent(e);
	  this.denotation.focus();
   },
   
   /** ce 
    * Called when the user clicked on the help button
    * @method onHelpButton
    */
   onHelpButton: function(e, args) {
	  if(new Date().getTime() - this.mousetime > 300) return; 
      Event.stopEvent(e);
	  var html = "";
	  if(this.help != "") html += "<p>" + this.help + "</p>";
	  if(this.helpurl != "") html += "<p><a href='" + this.helpurl + "' target='_blank'>" + this.helpurl + "</a></p>";
	  $('#help').parent().removeClass("hidden");
	  $('#help').parent().parent().children('a').addClass("active");
	  $('#help').html(html);
	  $('#help').animate({ backgroundColor: "#a7bf51"}, 500, function() {$('#help').animate({ backgroundColor: "#EDF5FF"}, 900 );} );
   },
   ...
   onCloseButton: function(e, args) {
      if(new Date().getTime() - this.mousetime > 300) return; //ce
      Event.stopEvent(e);
   ...
   reposition: function(x,y) { 	
	...
	var onTween = function() {
		container.redrawContainerWires();
		layer.layermap.draw();
		return !this.isAnimated();
	}

	var onComplete = function() {
		//ce use onComplete instead of in onTween if CPU load is to high
		//layer.layermap.draw();
	}
	
	var myAnim = new YAHOO.util.Anim(this.dd.id, {
   		top: { to: y },
		left:{ to: x } 
	    },2, YAHOO.util.Easing.easeOut);
	myAnim.onTween.subscribe( onTween );
	// see var onComplete 
	// myAnim.onComplete.subscribe( onComplete );
	myAnim.animate();
...
WireIt.Layer = function(options) {
   ...
   /**
    * ce 
	* focused Container array
	* array of all containers they are focused
    */
...
WireIt.Layer.prototype = {
   ...
   addWire: function(wireConfig) {
	  ...
	  //ce getContainer with ID
	  for(var i = 0; i < this.containers.length; i++){
		if(src.moduleId == this.containers[i].id) {
			var srcContainer = this.containers[i];
			break;
		}
	  }
	  for(var i = 0; i < this.containers.length; i++){
		if(tgt.moduleId == this.containers[i].id) {
			var tgtContainer = this.containers[i];
			break;
		}
	  }

      var terminal1 = srcContainer.getTerminal(src.terminal);
      var terminal2 = tgtContainer.getTerminal(tgt.terminal);
      ...
      for( i = 0 ; i < this.wires.length ; i++) {
         var wire = this.wires[i];
      	var wireObj = wire.getConfig();
			//ce wireObj.src = {moduleId: WireIt.indexOf(wire.terminal1.container, this.containers), terminal: wire.terminal1.name };
			//ce wireObj.tgt = {moduleId: WireIt.indexOf(wire.terminal2.container, this.containers), terminal: wire.terminal2.name };
			wireObj.src = {moduleId: wire.terminal1.container.id, terminal: wire.terminal1.name };
			wireObj.tgt = {moduleId: wire.terminal2.container.id, terminal: wire.terminal2.name };
...
YAHOO.lang.extend(WireIt.LayerMap, WireIt.CanvasElement, {
   ...
   initEvents: function() {
      ...
      Event.addListener(this.element, 'mouseout', this.onMouseOut, this, true);
	//ce Bugfix if mouse move out of LayerMap while dragging
	  Event.addListener(this.element, 'mouseover', this.onMouseEnter, this, true);
      //ce Updating the Minimap when resize the browser
	  Event.addListener(window, 'resize', this.draw, this, true);
   ...
   /** //ce
    * When a mouse leaves, save current situation in mouseLeft and set mousedown false
    * @method onMouseOut
    * @param {Event} e Original event
    * @param {Array} args event parameters
    */
   onMouseOut: function(e, args) { 
	 mouseLeft = this.isMouseDown;
	 this.isMouseDown = false;
   },  
   
   /** //ce
    * When a mouse enters, look if mouse is down
    * @method onMouseEnter
    * @param {Event} e Original event
    * @param {Array} args event parameters
    */
   onMouseEnter: function(e, args) { 
	 this.isMouseDown = mouseLeft;
   },  
    ...
    */
   onMouseMove: function(e, args) { 
	//ce without stopEvent SCORE#11 is solved
	// Event.stopEvent(e);
   ...
   onLayerScroll: function() {
      ...
      var that = this;
      this.scrollTimer = that.draw();//ce window.setTimeout(function() {
      //   that.draw();
      //},50);
   ...
   draw: function() {
      ...
      var canvasHeight = canvasRegion.bottom-canvasRegion.top -4;
      // Clear Rect //ce +4
      ctxt.clearRect(0,0, canvasWidth+4, canvasHeight+4);
   ...
   drawWires: function(ctxt, hRatio, vRatio) {
      ...
      for(i = 0 ; i < n ; i++) {
         wire = wires[i];
	//ce Don't draw a wire to TerminalProxy
	if(wire.terminal1.className == undefined || wire.terminal2.className == undefined){
		continue;
	}
         var pos1 = wire.terminal1.getXY(), 
             pos2 = wire.terminal2.getXY();
	
		//ce: Don't draw if position is ugly (maybe not needed since the continue above)
		if(pos2[0] != -1){
			 // Stroked line
			 // TODO:
			 ctxt.beginPath();
...
lang.extend(inputEx.Field, inputEx.BaseField, {
   ...
   setOptions: function(options) {
      ...
      this.options.ddConfig = lang.isUndefined(options.ddConfig) ? false : options.ddConfig;
	  this.options.nMaxWires = lang.isUndefined(options.nMaxWires) ? 9999 : options.nMaxWires;
	  
	  if(options.isXnode){
			this.options.terminalclassName = "WireIt-Terminal-X";
			this.options.terminalconnectedClassName = "WireIt-Terminal-connected-X";
			this.options.terminaldropinviteClassName = "WireIt-Terminal-dropinvite-X";
	  } else {
		this.options.terminalclassName = "WireIt-Terminal";
		this.options.terminalconnectedClassName = "WireIt-Terminal-connected";
		this.options.terminaldropinviteClassName = "WireIt-Terminal-dropinvite";
	  }
   ...
   renderTerminal: function() {
      ...
      this.terminal = new WireIt.Terminal(wrapper, {
         name: this.options.ddConfig.name, 
         direction: [0,-1],
         fakeDirection: [0, 1],
         ddConfig: this.options.ddConfig, // ce
		 
		 className: this.options.terminalclassName,//ce
		 connectedClassName: this.options.terminalconnectedClassName,//ce
		 dropinviteClassName: this.options.terminaldropinviteClassName,//ce
		
		 //{
         //   type: "input",
         //   allowedTypes: ["output"]
         //},
		nMaxWires: this.options.nMaxWires }, this.options.container);
      ...
      // Register the events
      //ce this.terminal.eventAddWire.subscribe(this.onAddWire, this, true);
      //ce this.terminal.eventRemoveWire.subscribe(this.onRemoveWire, this, true);
    },
...
YAHOO.lang.extend(WireIt.FormContainer, WireIt.Container, {
   ...
   setValue: function(val) {
      ...
      this.denotation.value = val.params.denotation; //ce added
      changeNoderuleType(this.form, val.selector.SelectorType); //ce added
   }
...
lang.extend(inputEx.Group, inputEx.Field, {
   ...
   renderFields: function(parentEl) {
	  ...
	  //ce
	  this.selector = inputEx.cn('div', {className: "selector"});
      ...
      for (var i = 0 ; i < this.options.fields.length ; i++) {
		 ...
		 if(input.inputParams != undefined){
			if (input.inputParams.name == "SelectorType"){
				selectorChild =true;
			}
		 }
  	   }
	   
	   //ce: Um alle SelectorFelder in ein div zu bekommen	 
		 if (selectorChild){
			for(var nodetypes in noderules){
				if(noderules[nodetypes] == "Headquarter_AssetType") continue;
				var selectorfield = this.renderField({"type": "select", "inputParams": {"label": noderules[nodetypes], "name": noderules[nodetypes], "selectValues": eval(noderules[nodetypes]+"IDs"), "selectOptions": eval(noderules[nodetypes])}});
				this.selector.appendChild(selectorfield.getEl() );
			}
			//Unnötige Felder ausblenden
			changeNoderuleType(this, "Headquarter");
		 }
	   ...
	   parentEl.appendChild(this.selector);
   },
   ...
   setValue: function(oValues, sendUpdatedEvt) {
      if(!oValues) {
	   ...
	   for (var i = 0 ; i < this.inputs.length ; i++) {
	      ...
	      } else if(name && !lang.isUndefined(oValues.params[name]) ) {                     //ce added because of params selector structure of Value
	         field.setValue(oValues.params[name], false); // don't fire the updated event !
	      } else if(name && !lang.isUndefined(oValues.selector[name]) ) {                   //ce added because of params selector structure of Value
	         field.setValue(oValues.selector[name], false); // don't fire the updated event !
	      } else {
	         field.clear(false);
	      }
      }
   ...
   getValue: function() {
	   var selector=false;
	   var p = {},s={}; //ce
	   var o = {params: p, selector:s}; //ce from {}
	   for (var i = 0 ; i < this.inputs.length ; i++) {
	      var v = this.inputs[i].getValue();
	      if(this.inputs[i].options.name) {
			if(this.inputs[i].options.name == "SelectorType") selector = true; //ce
	         if(this.inputs[i].options.flatten && lang.isObject(v) ) {
	            if(selector) lang.augmentObject( s, v); //ce
			    else lang.augmentObject( p, v); //ce o -> p
	         }
	         else {
		        if(selector) s[this.inputs[i].options.name] = v; //ce
				else p[this.inputs[i].options.name] = v; //ce o -> p
	         }
	      }
      }
	   return (selector) ? o : p;
   ...
   onChange: function(eventName, args) {
	  ...
	  if(fieldInstance.options.name == "SelectorType") changeNoderuleType(this, fieldValue);
...
WireIt.BaseEditor.defaultOptions = {
	layoutOptions: {
	 	units: [
	   	//ce { position: 'top', height: 57, body: 'top'},
	...
	propertiesFields: [
		{"type": "string", "name": "name", label: "Name", typeInvite: "Gib ein Titel ein", "value":"Defaultname" }, //ce: Verdeutscht
		{"type": "text", "name": "description", label: "Description", cols: 30, rows: 4}     //ce: Verdeutscht
	],
...
WireIt.BaseEditor.prototype = {
     ...
     */
	 ...
	 renderButtons: function() {
		...
		//ce added 
	    var buildButton = new widget.Button({ label:"Build", id:"WiringEditor-buildButton", container: toolbar });
	    buildButton.on("click", this.onBuild, this, true);

		//ce added 
	    var saveBuildButton = new widget.Button({ label:"SaveBuild", id:"WiringEditor-buildButton", container: toolbar });
	    saveBuildButton.on("click", this.onSaveBuild, this, true);

		//ce added 
	    var reorderButton = new widget.Button({ label:"Reorder", id:"WiringEditor-buildButton", container: toolbar });
	    reorderButton.on("click", this.onReorder, this, true);
	 },
	...
	inform: function(txt) {
		var html = "";
	  	if(this.help != "") html += "<p>" + txt + "</p>";
	  	$('#infos').parent().removeClass("hidden");
	  	$('#infos').parent().parent().children('a').addClass("active");
	  	$('#infos').html(html);
	  	$('#infos').animate({ backgroundColor: "#a7bf51"}, 500, function() {$('#infos').animate({ backgroundColor: "#EDF5FF"}, 900 );} );
	},

	/**
	 * Displays a message as alert
	 */
	alert: function(txt) {
...
lang.extend(WireIt.WiringEditor, WireIt.BaseEditor, {
	 ...
	 */
  	render: function() {
		...
		layer=this.layer; //ce: Um auf alle Objekte im Layer zugreifen zu können (anzeigen welche Terminals verbunden werden können) 
	...
	addModule: function(module, pos) {
	    try {
		   ...
		   containerConfig.vs_code = module.vs_code_id; //ce new
	       var temp = this;
 	...
 	save: function() {
		...
		//ce
		$('#propertiesForm').parent().removeClass("hidden");
	    $('#propertiesForm').parent().parent().children('a').addClass("active");
	    $('#propertiesForm > div > fieldset > div > .inputEx-Field > div > input').css('border', '2px solid red');
	...
	saveBuild: function() {
		...
		this.tempSavedWiring = {name: value.name, working: value.working, language: this.options.languageName };    
    		this.adapter.saveBuild(this.tempSavedWiring, {
       			success: this.saveModuleSuccess,
       			failure: this.saveModuleFailure,
       			scope: this
    		});
	...
	reorder: function() {
		...
		for(var i = 0 ; i < this.layer.wires.length ; i++) {
			dot += this.layer.wires[i].terminal1.container.id +"->"+ this.layer.wires[i].terminal2.container.id + ";";
		}
		for(var i = 0 ; i < this.layer.containers.length ; i++) {
			dot += this.layer.containers[i].id +";";
		}
    		this.adapter.reorder( dot, {
       			success: this.reorderSuccess,
       			failure: this.reorderFailure,
       			scope: this
    		});
 	},

	 /**
	  * reorder success callback
	  * @method reorderSuccess
	  */
	 reorderSuccess: function(o) {
		var obj = eval('('+o+')');
	   	for(var i = 0; i<obj.layout.length; i++){			
			for(var j = 0 ; j < this.layer.containers.length ; j++) {
				if(obj.layout[i].name == this.layer.containers[j].id) { this.layer.containers[j].reposition(obj.layout[i].x * 2, obj.layout[i].y * 2); }
			}
		}
	 },
	...
	inputFilterTimer: function() {
	 ...
	 updateLoadPanelList: function(filter) {
	    ...
	    Event.addListener(list, 'click', function(e,args) {
	    	console.log(Event.getTarget(e).innerHTML);
	 ...
	 getPipeByName: function(name) {
	    ...
	    for(var i = 0 ; i < n ; i++) {
	       if(this.pipes[i].name == name) {
				//ce set nextContainerID back to 1
				this.layer.nextContainerID = 1;
	 ...
	 loadPipe: function(name) {
		...
		//ce added JSON.parse line
		var wiring = JSON.parse(pipe);
		 if(!wiring) {
	    ...
	    if(lang.isArray(wiring.modules)) {
		   ...
		   //ce added id parameter to container configuration
		     m.config.id = m.id;
		     m.config.vs_code = m.vs_code;
...
console.log(m.value);
 ...
 getValue: function() {
   ...
   for( i = 0 ; i < this.layer.containers.length ; i++) {
	  //ce extract config to add denotation to it
	  var value = this.layer.containers[i].getValue();
	  if(this.layer.containers[i].denotation) value.params.denotation = this.layer.containers[i].denotation.value;
	  //ce added id attribute
      obj.modules.push( {id: this.layer.containers[i].id, name: this.layer.containers[i].title, vs_code: this.layer.containers[i].vs_code, value: value, config: this.layer.containers[i].getConfig()});
   }

   for( i = 0 ; i < this.layer.wires.length ; i++) {
      var wire = this.layer.wires[i];
		var wireObj = wire.getConfig();
		//ce wireObj.src = {moduleId: WireIt.indexOf(wire.terminal1.container, this.layer.containers), terminal: wire.terminal1.name };
